...
## ⚙️ Formulation of the L1 Optimization Framework

### 1️⃣ Objective
The task is to find the parameter vector  
\[
P = (\theta, M, X)
\]  
that minimizes the average distance between the empirical data points \((x_i, y_i)\) and the corresponding theoretical curve generated by the model:

\[
x(t),\; y(t) = f(t, P)
\]

The chosen metric for this distance is the **L1 norm** (mean absolute error), as specified in the assignment.

---

### 2️⃣ Why L1 Distance?
- **Robustness:** L1 is less sensitive to outliers than L2.  
- **Interpretability:** Represents the direct average deviation of predicted vs. observed points.  
- **Fair scoring:** Matches the assignment evaluation criterion (“L1 distance between uniformly sampled points”).

Mathematically, the L1 distance for N data points is:

\[
L(P) = \frac{1}{N} \sum_{i=1}^{N} \min_{t \in (6, 60)} \Big(|x_i - x(t, P)| + |y_i - y(t, P)|\Big)
\]

---

### 3️⃣ Practical Approximation (Chamfer Distance)
Computing the inner minimum for each point is expensive.  
To make the problem tractable, we approximate it using a **discretized Chamfer distance**:

1. Generate K uniform samples \(t_j\) in (6, 60).  
2. Compute the corresponding model points \(C_j = (x(t_j), y(t_j))\).  
3. For each data point \(D_i\), find the closest model point \(C_j\) in L1 distance.  
4. Average all minimum distances.

Formally:

\[
L(P) \approx \frac{1}{N} \sum_{i=1}^{N} \min_{j=1..K} \Big(|x_i - C_{x,j}| + |y_i - C_{y,j}|\Big)
\]

This approximation keeps the cost function continuous and efficient for optimization.

---

### 4️⃣ Algorithmic Steps
**Step 1:** Input the dataset `xy_data.csv`.  
**Step 2:** Generate 5000 uniform `t` samples in (6, 60).  
**Step 3:** For each candidate (θ, M, X), compute the model curve.  
**Step 4:** Evaluate L(P) using Chamfer distance.  
**Step 5:** Repeat for all parameter combinations suggested by the optimizer.  
**Step 6:** Select parameters with minimum L(P).  

---

### 5️⃣ Implementation Plan
- **Global Phase:** Use Differential Evolution (DE) to explore the full search space  
  \((0° < θ < 50°,\,-0.05 < M < 0.05,\,0 < X < 100)\).
- **Local Phase:** Use Nelder–Mead (Simplex Method) for fine-tuning around the global best point.
- **Evaluation:** Track L1 loss per iteration and visualize convergence.
...
| Parameter | Symbol | Expected Range | Interpretation |
|------------|:-------:|:---------------:|:---------------|
| **Theta (θ)** | θ | ≈ 25° | Direction angle of the curve |
| **Modulation (M)** | M | slightly < 0 | Damped oscillation |
| **Offset (X)** | X | ≈ 49 | Horizontal shift |

Final output → minimal L1 distance and corresponding parametric equation string for submission.
